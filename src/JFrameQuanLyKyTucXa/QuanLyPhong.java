...
public class QuanLyPhong extends javax.swing.JPanel {
                                ...
                                .addGroup(AddPhongLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                                    ...
                                    .addGroup(javax.swing.GroupLayout.Alignment.TRAILING, AddPhongLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                                        .addGroup(AddPhongLayout.createSequentialGroup()
                                            ...
                                            .addGap(44, 44, 44)
                                            .addComponent(rdNu))
                                        .addComponent(txtAddTienPhong, javax.swing.GroupLayout.PREFERRED_SIZE, 280, javax.swing.GroupLayout.PREFERRED_SIZE)))
    ...
    private void editPhongMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_editPhongMouseClicked
        // TODO add your handling code here:
        if (count == -1) {
            JOptionPane.showMessageDialog(this, "Vui lòng chọn phòng để update!");
        } else {
    ...
    private void deletePhongMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_deletePhongMouseClicked
        ...
        if (count == -1) {
            ...
            int soluongsv = new PhongDAO().SoLuongSV(maPhong);
            if (soluongsv > 0) {
                JOptionPane.showMessageDialog(this, "Không thể xóa phòng vì sinh viên còn trong phòng!");
            } else {
                ...
                if (test == JOptionPane.YES_OPTION) {
                    deletePhong(maPhong);
                    deleteDichVu(maPhong);
                } else if (test == JOptionPane.NO_OPTION) {
                    JOptionPane.showMessageDialog(this, "Bạn đã hủy xóa phòng thành công");
                }
            }
        }
    ...
    private void btnUpdatePhongMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_btnUpdatePhongMouseClicked
        ...
        if (tenphong.isEmpty()) {
            JOptionPane.showMessageDialog(this, "Không để trống tên phòng. Vui lòng điền đầy đủ thông tin!");
        } else if (soluongsv.isEmpty()) {
            JOptionPane.showMessageDialog(this, "Không để trống số lượng sinh viên. Vui lòng điền đầy đủ thông tin!");
        } else if (tienphong.isEmpty()) {
            JOptionPane.showMessageDialog(this, "Không để trống tiền phòng. Vui lòng điền đầy đủ thông tin!");
        } else if (!soluongsv.matches(kiemtraso)) {
            JOptionPane.showMessageDialog(this, "Định dạng số sinh viên sai. Vui lòng nhập lại!");
        } else if (!tienphong.matches(kiemtraso)) {
            JOptionPane.showMessageDialog(this, "Định dạng số tiền phòng sai. Vui lòng nhập lại!");
        } else {
            int soluong = Integer.parseInt(txtUpSoLuongSinhVienPhong.getText().trim());
            if (soluongsvphong > soluong) {
                JOptionPane.showMessageDialog(this, "Hiện tại số sinh viên vượt qua số lượng sinh viên của phòng. Vui lòng nhập lại số lượng sinh viên của phòng lớn hơn!");
            } else {
                ...
                if (test == JOptionPane.YES_OPTION) {
                    LayMaPhong(count);
                    updatePhong(maPhong);
                    updateHoaDonSV(maPhong);
                    JOptionPane.showMessageDialog(this, "Bạn cập nhật phòng thành công!");
                } else if (test == JOptionPane.NO_OPTION) {
    ...
    private void btnAddSVMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_btnAddSVMouseClicked
        ...
        String soluongsv = txtAddSoLuongSV.getText().trim();
        String tienphong = txtAddTienPhong.getText().trim();

        String kiemtraso = "^[0-9.]+$";
        boolean kiemtramaphong = new PhongDAO().KiemTraMaSoPhong(maphong);

        if (maphong.isEmpty()) {
            JOptionPane.showMessageDialog(this, "Không để trống mã phòng. Vui lòng điền đầy đủ thông tin!");
            ...
            JOptionPane.showMessageDialog(this, "Mã phòng đã được đăng ký!");
        } else if (!soluongsv.matches(kiemtraso)) {
            JOptionPane.showMessageDialog(this, "Định dạng số sinh viên sai. Vui lòng nhập lại!");
        } else if (!tienphong.matches(kiemtraso)) {
            JOptionPane.showMessageDialog(this, "Định dạng số tiền phòng sai. Vui lòng nhập lại!");
    ...
    void showTable() {
        ...
        for (ThongTinPhong phong : listphong) {
            ...
            Model.addRow(new Object[]{
                phong.getMaPhong(), phong.getTenPhong(), phong.getSoLuongSVPhong(), soluongsv, phong.getLoaiPhong(), phong.getTienPhong()
            });
        }
        count = -1;
    }
    ...
    public void AddPhong() {
        ...
        String sql = "insert into thongtinphong (maPhong,tenPhong,soLuongSVPhong,loaiPhong,tienPhong,gioiTinh)"
                + "values(?,?,?,?,?,?)";
        int row = 0;
        try {
    ...
    public void AddDichVuPhong() {
        Connection conn = KetNoiSQL.getConnection();
        float so = 0;
        double tien = 0;
        String sql = "insert into thongtinhoadon (maPhong,soNuocMoi,soNuocCu,soDienMoi,soDienCu,tienThanhToan)"
                + "values(?,?,?,?,?,?)";
        try {
            PreparedStatement ps = conn.prepareStatement(sql);
    ...
    private void deleteDichVu(String maPhong) {
        Connection conn = KetNoiSQL.getConnection();
        String sql = "delete from thongtinhoadon where maPhong = ?";
        try {
            PreparedStatement ps = conn.prepareStatement(sql);
            ps.setString(1, maPhong);
            ps.executeUpdate();
    ...
    private void updatePhong(String maPhong) {
        ...
        Connection conn = KetNoiSQL.getConnection();
        String sql = "update thongtinphong set tenPhong='" + tenPhong + "', soLuongSVPhong='" + soluongsv + "', loaiPhong='" + selectedUpLoaiPhong + "' ,tienPhong='" + tienphong + "' where maPhong='" + maPhong + "'";
        try {
            PreparedStatement ps = conn.prepareStatement(sql);
            ps.executeUpdate();
            conn.close();
    ...
    private void updateHoaDonSV(String maph) {
        double tienphi = new ThongTinHoaDonDAO().TienThanhToan(maph);

        Connection conn = KetNoiSQL.getConnection();
        String sql = "update thongtinhoadonsv set tienPhi=? where maPhong=?";
        try {
            PreparedStatement ps = conn.prepareStatement(sql);
