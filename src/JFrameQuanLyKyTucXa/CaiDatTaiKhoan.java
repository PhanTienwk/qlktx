...
public class CaiDatTaiKhoan extends javax.swing.JPanel {
    ...
    private void jButton1MouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_jButton1MouseClicked
        ...
        boolean kiemtratendn = new TaiKhoanDAO().KiemTraTenDNSV(tendangnhap);
        boolean kiemtraemail = new SinhVienDAO().KiemTraEmailSV(email);
        boolean kiemtraemaildk = new SinhVienDangKyDAO().KiemTraEmailSV(email);
        boolean kiemtraemailnv = new QuanLyNhanVienDAO().KiemTraEmailSV(email);
        ...
        if (email.isEmpty()) {
            ...
            if (test1 == JOptionPane.YES_OPTION) {
                ...
                if (kiemtraemail) {
                    DoiLaiTenDN();
                } else if (kiemtraemaildk) {
                    DoiLaiTenDNDK();
                } else if (kiemtraemailnv) {
                    DoiLaiTenDNNV();
    ...
    void XacNhanTaiKhoan() {
        ...
        String sql = "select * from thongtintaikhoan where tenDangNhap='" + tendangnhap + "' and matKhau='" + matkhau + "' and email ='" + email + "'";
        try {
    ...
    void DoiLaiMatKhau() {
        if (test == true) {
            Connection conn = KetNoiSQL.getConnection();
            int row = 0;
            String sql1 = "update thongtintaikhoan set tenDangNhap=?, matKhau=? where email=?";
            try {
    ...
    void DoiLaiTenDN() {
        if (test) {
            Connection conn = KetNoiSQL.getConnection();
            int row = 0;
            String sql1 = "update thongtinsinhvien set tenDangNhap=? where email=?";
            try {
                PreparedStatement ps1 = conn.prepareStatement(sql1);
    ...
    void DoiLaiTenDNDK() {
        if (test) {
            Connection conn = KetNoiSQL.getConnection();
            int row = 0;
            String sql1 = "update thongtinsinhviendangky set tenDangNhap=? where email=?";
            try {
                PreparedStatement ps1 = conn.prepareStatement(sql1);
    ...
    void DoiLaiTenDNNV() {
        if (test) {
            Connection conn = KetNoiSQL.getConnection();
            int row = 0;
            String sql1 = "update thongtinnhanvien set tenDangNhap=? where email=?";
            try {
                PreparedStatement ps1 = conn.prepareStatement(sql1);
